...
class Lisp:
    ...
    def eq(self, env, args):
        if len(args) > 2:
            raise ValueError(
                "Wrong number of arguments, expected {0}, got {1}".format(2, len(args))
            )
    ...
    def quote(self, env, args):
        if len(args) > 1:
            raise ValueError(
                "Wrong number of arguments, expected {0}, got {1}".format(1, len(args))
            )
    ...
    def car(self, env, args):
        ...
        if not isinstance(cell, Seq):
            raise ValueError(
                "Function car not valid on non-sequence type: %s" % cell.data
            )
    ...
    def cdr(self, env, args):
        ...
        if not isinstance(cell, Seq):
            raise ValueError(
                "Function cdr not valid on non-sequence type: %s" % cell.data
            )
